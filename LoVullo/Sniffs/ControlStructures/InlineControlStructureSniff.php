 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class LoVullo_Sniffs_ControlStructures_InlineControlStructureSniff implements PHP_CodeSniffer_Sniff
{
    /**
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['parenthesis_closer'])) {
            // If next token after parenthesis is a colon, then this is an alternate form
            // statement and is allowed
            if ($tokens[$tokens[$stackPtr]['parenthesis_closer'] + 1]['code'] === T_COLON) {
                return;
            }
        } elseif ($tokens[$stackPtr]['code'] === T_ELSE) {
            // If next token after the ELSE is a colon, then this is an alternate form
            // statement and is allowed
            if ($tokens[$stackPtr + 1]['code'] === T_COLON) {
                return;
            }
        }
